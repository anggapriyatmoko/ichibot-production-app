...
export async function GET(
    ...
    // Add hr-docs subfolder
    const hrDocsDir = path.join(uploadDir, 'hr-docs')
    const filePath = path.join(hrDocsDir, filename)

    // Security check: Prevent directory traversal
    const resolvedPath = path.resolve(filePath)
    if (!resolvedPath.startsWith(path.resolve(hrDocsDir))) {
