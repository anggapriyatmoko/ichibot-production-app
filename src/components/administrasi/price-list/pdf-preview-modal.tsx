...
import { generateProductPdf } from "./generate-pdf"
...
export default function PriceListItemPdfModal({
    isOpen,
    onClose,
    item
}: PdfModalProps) {
    ...
    useEffect(() => {
        if (isOpen && item) {
            ...
            const generate = async () => {
                try {
                    const result = await generateProductPdf(item, true) as any
            ...
            // Artificial delay for better UX if generation is too fast
            // and to allow UI to render first
    ...
    const handleDownload = () => {
        ...
        link.href = pdfUrl
        link.download = `Detail-Produk-${item.name.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.pdf`
